...
/// fixed list that does not support overflow.
#[repr(C)]
#[allow(private_bounds)]
pub struct FixedList<T, const NODE_COUNT: usize, OverflowAllocator: Allocator>
where
    [(); ((NODE_COUNT + 1) * mem::size_of::<ListNode<T>>()) - 1]:,
{
    base_list: List<T, FixedPoolWithOverflow<ListNode<T>, OverflowAllocator>>,
    // we allocate (node size - 1) extra bytes to ensure that no matter the alignment of `buffer`,
    // we can fit `NODE_COUNT` nodes in the fixed buffer area
    buffer: [u8; ((NODE_COUNT + 1) * mem::size_of::<ListNode<T>>()) - 1],
}
    ...
    [(); ((NODE_COUNT + 1) * mem::size_of::<ListNode<T>>()) - 1]:,
{
    ...
    pub unsafe fn new_in(allocator: OverflowAllocator) -> impl New<Output = Self> {
        ...
        .with(|this| {
            ...
            this.base_list.allocator.init(&mut this.buffer);
        })
    }
